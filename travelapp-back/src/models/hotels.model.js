...
async function getAllHotels(filter, sort, sortBy) {
    let hotels = Hotel.find(filter)
    if (sort && sortBy && sortBy == 'stars' && (sort == 'asc' || sort == 'desc')) {
        hotels = await hotels.sort({ stars: sort })
    }
    return hotels
}
