function chatData(chat) {
    ...
    if (chat.messages.length > 0) {
        name = chat.messages[chat.messages.length - 1].sender_id.name.first_name + ' ' + chat.messages[chat.messages.length - 1].sender_id.name.last_name
        last_message = `${name}: ${chat.messages[chat.messages.length - 1].content}`
    }
    return {
        id: chat.trip_id._id,
        chat_name: chat.name,
